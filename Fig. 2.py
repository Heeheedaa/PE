...
results_df = pd.DataFrame(columns=['Scenarios', 'RMSE', 'R2', 'Bias','Slope','MAPE'])


#------------------------------------------------------------------------------------------------------------------#
#XGBoost model climate
...
new_columns = ['SWC',  'CO$_2$',  'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    #results_df = results_df.append({'Scenarios': 'C (Climate, baseline)', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C (Climate, baseline)', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
X = merged_df[['SWC_F_MDS_1', 'LCC', 'CO2_F_MDS',  'aridity', 'TA_F', 'VPD_F', 'CI']]

# The new column names
new_columns = ['SWC', 'Chl', 'CO$_2$',  'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + Chl', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + Chl', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
X = merged_df[['SWC_F_MDS_1', 'LAI', 'CO2_F_MDS',  'aridity', 'TA_F', 'VPD_F', 'CI']]

# The new column names
new_columns = ['SWC', 'LAI', 'CO$_2$',  'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + LAI', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + LAI', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
#------------------------------------------------------------------------------------------------------------------#
#XGBoost model Climate + RS (CI_clump)
...
X = merged_df[['SWC_F_MDS_1', 'daily_clumping', 'CO2_F_MDS',  'aridity', 'TA_F', 'VPD_F', 'CI']]

# The new column names
new_columns = ['SWC', 'CI$_{clump}$', 'CO$_2$',  'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + CI$_{clump}$', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + CI$_{clump}$', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
#------------------------------------------------------------------------------------------------------------------#
#XGBoost model Climate + RS (Chl, LAI, CI_clump)
...
X = merged_df[['SWC_F_MDS_1', 'LCC', 'CO2_F_MDS', 'daily_clumping',  'LAI', 'aridity', 'TA_F', 'VPD_F', 'CI']]

# The new column names
new_columns = ['SWC', 'Chl', 'CO$_2$', 'CI$_{clump}$', 'LAI',  'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + RS (all)', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + RS (all)', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
X = merged_df[['SWC_F_MDS_1',  'CO2_F_MDS',    'LL',  'aridity', 'TA_F', 'VPD_F', 'CI']]

# The new column names
new_columns = ['SWC', 'CO$_2$',    'LL',   'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + LL', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + LL', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
X = merged_df[['SWC_F_MDS_1',  'CO2_F_MDS',    'LMA',  'aridity', 'TA_F', 'VPD_F', 'CI']]

# The new column names
new_columns = ['SWC', 'CO$_2$',    'LMA',   'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + LMA', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + LMA', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
#------------------------------------------------------------------------------------------------------------------#
#XGBoost model Climate + EEO (Vcmax)
...
X = merged_df[['SWC_F_MDS_1',  'CO2_F_MDS',    'Vcmax25',  'aridity', 'TA_F', 'VPD_F', 'CI']]

# The new column names
new_columns = ['SWC', 'CO$_2$',   'V$_{cmax25}$',   'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + V$_{cmax25}$', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + V$_{cmax25}$', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
#------------------------------------------------------------------------------------------------------------------#
#XGBoost model Climate + EEO (LL, LMA, Vcmax25)
...
X = merged_df[['SWC_F_MDS_1',  'CO2_F_MDS', 'LMA',   'LL',  'Vcmax25',  'aridity', 'TA_F', 'VPD_F', 'CI']]

# The new column names
new_columns = ['SWC', 'CO$_2$',  'LMA',   'LL',  'V$_{cmax25}$',   'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + EEO (all)', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + EEO (all)', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
# The new column names
new_columns = ['SWC', 'Chl', 'CO$_2$', 'CI$_{clump}$', 'LMA',   'LL',  'V$_{cmax25}$', 'LAI',  'Aridity','T$_a$', 'VPD', 'CI$_{cloud}$',]
...
for train_index, test_index in kf.split(X):
    ...
    # Append results to DataFrame
    #results_df = results_df.append({'Scenarios': 'C + RS + EEO', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias':bias, 'MAPE': mape}, ignore_index=True)
    results_df = pd.concat([results_df, pd.DataFrame([{'Scenarios': 'C + RS + EEO', 'Slope': slope, 'RMSE': rmse, 'R2': r2, 'Bias': bias, 'MAPE': mape}])], ignore_index=True)
...
colors = ['lightgrey'] + ['mediumseagreen'] * 4 + ['orange'] * 4 + ['dimgray']

plt.subplot(2, 2, 1)

# Boxplot for R2
sns.violinplot(x='Scenarios', y='R2', data=results_df, palette=colors,  inner=None )
sns.boxplot(x='Scenarios', y='R2', data=results_df, color="white",width=0.15,fill=False,whis=(0, 100),linecolor='white')
#sns.swarmplot(x='Scenarios', y='R2',data=results_df, color="white", edgecolor="white",size=3.5)

plt.ylabel('R$^{2}$',fontsize=20)
plt.title('Variation explained', fontsize=20,color='black')
#plt.axhline(y=0.8, color='#4ecdc4', linestyle='--')

plt.text(-0.1, 0.42, 'Climate (baseline)', color='grey', fontsize=16, rotation=90, )
plt.text(0.9, 0.54, '+ Chl', color='gray', fontsize=16, rotation=90, )
plt.text(1.9, 0.54, '+ LAI', color='gray', fontsize=16, rotation=90, )
plt.text(2.9, 0.51, '+ CI$_{clump}$', color='gray', fontsize=16, rotation=90, )
plt.text(3.9, 0.46, '+ RS (all)', color='mediumseagreen', fontsize=16, rotation=90,  )
plt.text(4.9, 0.50, '+ LL', color='gray', fontsize=16, rotation=90, )
plt.text(5.9, 0.50, '+ LMA', color='gray', fontsize=16, rotation=90, )
plt.text(6.9, 0.53, '+ V$_{cmax25}$', color='gray', fontsize=16, rotation=90, )
plt.text(7.9, 0.56, '+ EEO (all)', color='orange', fontsize=16, rotation=90, )
plt.text(8.9, 0.48, '+ RS (all) & EEO (all)', color='black', fontsize=16, rotation=90)

plt.annotate('', xy=(0.8, 0.68), xytext=(4.2, 0.68), xycoords='data', textcoords='data',
             arrowprops={'arrowstyle': '-[', 'lw': 1, 'color': 'seagreen'})
plt.text(2.5, 0.69, 'RS', color='mediumseagreen', fontsize=16, ha='center', fontweight='bold')

plt.annotate('', xy=(4.8, 0.78), xytext=(8.2, 0.78), xycoords='data', textcoords='data',
             arrowprops={'arrowstyle': '-[', 'lw': 1, 'color': 'orange'})
plt.text(6.5, 0.79, 'EEO', color='orange', fontsize=16, ha='center', fontweight='bold')
...
sns.violinplot(x='Scenarios', y='RMSE', data=results_df, palette=colors, inner=None)
sns.boxplot(x='Scenarios', y='RMSE', data=results_df, color="white",width=0.15,fill=False,whis=(0, 100),linecolor='white')

#sns.swarmplot(x='Scenarios', y='RMSE',data=results_df, color="white", edgecolor="white",size=3.5)
plt.ylabel('RMSE ($mol$ $C$ $mol$ $^{−1}$ $photons$)',fontsize=20)
#plt.ylabel(f'RMSE = {rmse:.4f} ({(rmse/np.mean(y_test))*100:.2f}%)',  fontsize=16)
#plt.axhline(y=0.0064, color='#ff6b6b', linestyle='--')
#plt.text(2.0, 0.0065, 'General model', color='#ff6b6b', fontsize=12, ha='center')



plt.title('Root mean square error', fontsize=20,color='black')
#plt.text(-0.05, 1.15, 'b', transform=plt.gca().transAxes, fontsize=24, va='top', ha='left', fontweight='bold')    
print(f"XGBoost Average RMSE: {average_rmse}")
print(f"XGBoost Average R-squared: {average_r2}")
plt.gca().spines['right'].set_visible(False)
plt.gca().spines['top'].set_visible(False)
...
plt.tick_params(axis='both', which='both', length=6, width=2, labelsize=14)
#plt.gca().tick_params(axis='x', pad=-200, )    

plt.text(-0.1, 0.0074, 'Climate (baseline)', color='grey', fontsize=16, rotation=90, )
plt.text(0.9, 0.0084, '+ Chl', color='gray', fontsize=16, rotation=90, )
plt.text(1.9, 0.0086, '+ LAI', color='gray', fontsize=16, rotation=90, )
plt.text(2.9, 0.0088, '+ CI$_{clump}$', color='gray', fontsize=16, rotation=90, )
plt.text(3.9, 0.0095, '+ RS (all)', color='mediumseagreen', fontsize=16, rotation=90, )
plt.text(4.9, 0.0098, '+ LL', color='gray', fontsize=16, rotation=90, )
plt.text(5.9, 0.0093, '+ LMA', color='gray', fontsize=16, rotation=90, )
plt.text(6.9, 0.008, '+ V$_{cmax25}$', color='gray', fontsize=16, rotation=90, )
plt.text(7.9, 0.0076, '+ EEO (all)', color='orange', fontsize=16, rotation=90, )
plt.text(8.9, 0.0069, '+ RS (all) & EEO (all)', color='black', fontsize=16, rotation=90)



plt.annotate('', xy=(0.8, 0.0078), xytext=(4.2, 0.0078), xycoords='data', textcoords='data',
             arrowprops={'arrowstyle': '-[', 'lw': 1, 'color': 'green'})
plt.text(2.5, 0.0079, 'RS', color='mediumseagreen', fontsize=16, ha='center', fontweight='bold')

plt.annotate('', xy=(4.8, 0.0067), xytext=(8.2, 0.0067), xycoords='data', textcoords='data',
             arrowprops={'arrowstyle': '-[', 'lw': 1, 'color': 'orange'})
plt.text(6.5, 0.0068, 'EEO', color='orange', fontsize=16, ha='center', fontweight='bold')
...
sns.violinplot(x='Scenarios', y='Slope', data=results_df, palette=colors, inner=None)
sns.boxplot(x='Scenarios', y='Slope', data=results_df, color="white",width=0.15,fill=False,whis=(0, 100),linecolor='white')

#sns.swarmplot(x='Scenarios', y='Slope',data=results_df, color="white", edgecolor="white",size=3.5)
...
#plt.text(-0.05, 1.15, 'c', transform=plt.gca().transAxes, fontsize=24, va='top', ha='left', fontweight='bold')    
print(f"XGBoost Average RMSE: {average_rmse}")
print(f"XGBoost Average R-squared: {average_r2}")
plt.gca().spines['right'].set_visible(False)
plt.gca().spines['top'].set_visible(False)
...
plt.text(-0.1, 0.905, 'Climate (baseline)', color='grey', fontsize=16, rotation=90, )
plt.text(0.9, 0.96, '+ Chl', color='gray', fontsize=16, rotation=90, )
plt.text(1.9, 0.96, '+ LAI', color='gray', fontsize=16, rotation=90, )
plt.text(2.9, 0.96, '+ CI$_{clump}$', color='gray', fontsize=16, rotation=90, )
plt.text(3.9, 0.84, '+ RS (all)', color='mediumseagreen', fontsize=16, rotation=90, )
plt.text(4.9, 0.86, '+ LL', color='gray', fontsize=16, rotation=90, )
plt.text(5.9, 0.85, '+ LMA', color='gray', fontsize=16, rotation=90, )
plt.text(6.9, 0.78, '+ V$_{cmax25}$', color='gray', fontsize=16, rotation=90,)
plt.text(7.9, 0.84, '+ EEO (all)', color='orange', fontsize=16, rotation=90, )
plt.text(8.9, 0.74, '+ RS (all) & EEO (all)', color='black', fontsize=16, rotation=90, )



plt.annotate('', xy=(0.8, 0.8), xytext=(4.2, 0.8), xycoords='data', textcoords='data',
             arrowprops={'arrowstyle': '-[', 'lw': 1, 'color': 'green'})
plt.text(2.5, 0.81, 'RS', color='mediumseagreen', fontsize=16, ha='center', fontweight='bold')

plt.annotate('', xy=(4.8, 1.06), xytext=(8.2, 1.06), xycoords='data', textcoords='data',
             arrowprops={'arrowstyle': '-[', 'lw': 1, 'color': 'orange'})
plt.text(6.5, 1.07, 'EEO', color='orange', fontsize=16, ha='center', fontweight='bold')
...
sns.violinplot(x='Scenarios', y='MAPE', data=results_df, palette=colors, inner=None)
sns.boxplot(x='Scenarios', y='MAPE', data=results_df, color="white",width=0.15,fill=False,whis=(0, 100),linecolor='white')

#sns.swarmplot(x='Scenarios', y='MAPE',data=results_df, color="white", edgecolor="white",size=3.5)
...
plt.text(-0.1, 40.5, 'Climate (baseline)', color='grey', fontsize=16, rotation=90, )
plt.text(0.9, 45, '+ Chl', color='gray', fontsize=16, rotation=90, )
plt.text(1.9, 45, '+ LAI', color='gray', fontsize=16, rotation=90, )
plt.text(2.9, 48, '+ CI$_{clump}$', color='gray', fontsize=16, rotation=90, )
plt.text(3.9, 54, '+ RS (all)', color='mediumseagreen', fontsize=16, rotation=90, )
plt.text(4.9, 59, '+ LL', color='gray', fontsize=16, rotation=90, )
plt.text(5.9, 58, '+ LMA', color='gray', fontsize=16, rotation=90, )
plt.text(6.9, 42, '+ V$_{cmax25}$', color='gray', fontsize=16, rotation=90, )
plt.text(7.9, 44, '+ EEO (all)', color='orange', fontsize=16, rotation=90, )
plt.text(8.9, 36, '+ RS (all) & EEO (all)', color='black', fontsize=16, rotation=90,)



plt.annotate('', xy=(0.8, 40), xytext=(4.2, 40), xycoords='data', textcoords='data',
             arrowprops={'arrowstyle': '-[', 'lw': 1, 'color': 'green'})
plt.text(2.5, 41, 'RS', color='mediumseagreen', fontsize=16, ha='center', fontweight='bold')

plt.annotate('', xy=(4.8, 35), xytext=(8.2, 35), xycoords='data', textcoords='data',
             arrowprops={'arrowstyle': '-[', 'lw': 1, 'color': 'orange'})
plt.text(6.5, 36, 'EEO', color='orange', fontsize=16, ha='center', fontweight='bold')
...
for i, ax in enumerate(axs.flatten()):
    ...
    if i < len(annotations):
        ax.text(-0.11, 1.09, annotations[i], transform=ax.transAxes, fontsize=24, va='top', ha='left', fontweight='bold')
